...
RSpec.describe Post, type: :model do
  ...
  describe '#update_post_counter' do
    ...
    it "updates user's posts_counter after destroying a post" do
      post = user2.posts.create(title: 'Test', likes_counter: 0, comments_counter: 0)
      expect do
        post.destroy
        user2.reload
      end.to change(user2, :posts_counter).by(-1)
    end
  end
end
