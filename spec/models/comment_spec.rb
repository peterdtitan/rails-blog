...
RSpec.describe Comment, type: :model do
  describe '#update_comment_counter' do
    ...
    it "updates post's comments_counter after destroying a comment" do
      comment = post2.comments.create(text: 'Test', author_id: user.id)
      expect do
        comment.destroy
        post2.reload
      end.to change(post2, :comments_counter).by(-1)
    end
  end
end
